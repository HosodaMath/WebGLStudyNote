...
vec2 waveEffect(vec2 uv){
  uv.x += 0.0035 * sin(uv.y * 100.0 + (vTime * 0.05) * PI2);
  uv.x += 0.0015 * cos(uv.y * 250.0 + (vTime * 0.05) * PI2);
  
  uv.y += 0.0035 * sin(uv.x * 100.0 + (vTime * 0.05) * PI2);
  uv.y += 0.0015 * cos(uv.x * 250.0 + (vTime * 0.05) * PI2);
...
void main(void){
  ...
  vec2 uFrequency = vec2(10.0, 10.0);
  vec2 uAmplitude = vec2(0.25, 0.25);
  float cosWave = cos(uv.y * uFrequency.x + vTime * 0.05) * uAmplitude.x;
  float sinWave = sin(uv.x * uFrequency.y + vTime * 0.05) * uAmplitude.y;
  uv.x = uv.x + cosWave;
  uv.y = uv.y + sinWave;
