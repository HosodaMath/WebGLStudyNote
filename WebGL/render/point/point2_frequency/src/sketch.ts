...
import { uniformController } from "./controller/uniform/unifromController";
import PointVertexShader from "./shader/point/point_frequency.vert?raw";
import PointFragmentShader from "./shader/point/point_frequency.frag?raw";
...
export const pointSketch = (
  ...
  */
  ...
  const draw = () => {
    ...
    const uMatrixData = uniformLocationData.get("uModelViewProjectionMatrix");
    if (!uMatrixData) {
      throw new Error("Error!! データが未定義です");
    }
    gl.uniformMatrix4fv(uMatrixData, false, uModelViewProjectionMatrix);
