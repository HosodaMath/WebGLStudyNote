...
import { Matrix4, Vector2, Vector3, random } from "../../lib/math/mathematics";
 ...
 * @param mainBody
 */
export const sketch2 = (mainBody: HTMLElement) => {
  const canvas = document.createElement("canvas");
  canvas.classList.add(...["canvas", "webGLCanvas"]);
  // 一時的に非表示
  ...
  // 振動数
  let uFrequency = Vector2.set(random(-20, 20), random(-20, 20));

  // 振幅
  let uAmplitude = Vector2.set(random(-0.5, 0.5), random(-0.5, 0.5));
  ...
  const animation = () => {
    // Color Program
    updateClearColor(width, height, gl, [0.2, 0.22, 0.22]);
