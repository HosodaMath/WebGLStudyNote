/***
 * 環境光 ambientLighting
 * 自然にある光の乱反射をシミュレーションする。
 * 平行光源の欠点である光が乱反射しないという現象を補う。
 * 光の乱反射をシミュレーションしているので値は大きすぎないほうが良い。
 * 大きいと白くなってしまう。
 * 環境光と拡散光によるシミュレーション
*/
...
import AmbientLightVertexShader from "./shader/light/ambient_light.vert";
import AmbientLightFragmentShader from "./shader/light/ambient_light.frag";
import "sanitize.css";
import "./main.css";
window.addEventListener("DOMContentLoaded", () => {
  ...
  const init = () => {
    ...
    const rectVertexShader = initShader(
      gl,
      "VERTEX_SHADER",
      AmbientLightVertexShader
    );
    const rectFragmentShader = initShader(
      gl,
      "FRAGMENT_SHADER",
      AmbientLightFragmentShader
    );
