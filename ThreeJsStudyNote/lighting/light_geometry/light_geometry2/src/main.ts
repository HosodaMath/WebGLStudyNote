...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const animation = () => {
    ...
    pointLight2.position.x = Math.cos(-1 * time) * 20 + 5;
    pointLight2.position.z = Math.sin(-1 * time) * 20;
    const pointLight3 = scene.getObjectByName("pointLight3");
    pointLight3.position.x = Math.cos(time) * 20 - 5;
