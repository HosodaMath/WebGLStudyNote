...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const COLOR_PALLET = [0x6879ba, 0x006cb8, 0x00377e, 0x5a9b91, 0x04b3a9];
  ...
  const init = () => {
    ...
    [...Array(300).keys()].forEach((count) => {
      const INIT_POSITION = new THREE.Vector3(
        Mathematics.random(-500, 500),
        Mathematics.random(-500, 500),
        Mathematics.random(-500, 0)
      );

      const CHOICE_COLOR = Math.floor(
        Mathematics.random(0, COLOR_PALLET.length)
      );

      const COLORS = COLOR_PALLET[CHOICE_COLOR];

      create_shape(INIT_POSITION, COLORS);
    });
