...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const createCanvasTexture = () => {
    ...
    /*
    grad.addColorStop(0.0, "rgba(0, 0, 100, 1.0)");
    grad.addColorStop(0.5, "rgba(100, 100, 100, 1.0)");
    grad.addColorStop(1.0, "rgba(200, 200, 240, 1.0)");
    */

    grad.addColorStop(0.0, "rgba(0, 0, 30, 1.0)");
    grad.addColorStop(0.5, "rgba(80, 70, 100, 1.0)");
    grad.addColorStop(1.0, "rgba(100, 100, 180, 1.0)");

    gl.fillStyle = grad;
    gl.rect(0, 0, canvasSize.x, canvasSize.y);
    gl.fill();

    const mapBackground = new THREE.CanvasTexture(canvas);

    const geometryBackground = new THREE.PlaneBufferGeometry(canvasSize.x, canvasSize.y);
