...
window.addEventListener("DOMContentLoaded", () => {
  ...
  /// ボックスマテリアルの作成 LambertMateriallを使用
  ...
  let sphereLight2 = new THREE.PointLight(0xffffff, 2, 50);
  sphereLight2.add(
    new THREE.Mesh(
      SPHERE_GEOMETRY,
      new THREE.MeshBasicMaterial({ color: 0xffffff })
  ...
  let sphereLight3 = new THREE.PointLight(0xffffff, 2, 50);
  sphereLight3.add(
    new THREE.Mesh(
      SPHERE_GEOMETRY,
      new THREE.MeshBasicMaterial({ color: 0xffffff })
  ...
  const draw = (): void => {
    ...
    sphereLight2.position.z = Math.cos(TIME * 0.05) * 30;
    sphereLight2.position.y = Math.sin(TIME * 0.05) * 30;

    sphereLight3.position.x = Math.cos(TIME * 0.05) * 30;
    sphereLight3.position.y = Math.sin(TIME * 0.05) * 30;
    sphereLight3.position.z = Math.cos(TIME * 0.02) * 30;
    // シーンとカメラを表示
